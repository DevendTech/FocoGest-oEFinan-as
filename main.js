...
function carregarAvaliacoesGoogle() {
  const placeID = 'ChIJTSy6T1NrqwcR08GHPj6kSLI'; // Substitua pelo seu Place ID
    ...
    if (status === google.maps.places.PlacesServiceStatus.OK) {
      ...
      if (place.reviews && place.reviews.length > 0) {
        place.reviews.forEach((review) => {
          // Cria um elemento para cada avaliação
          const reviewElement = document.createElement("div");
          reviewElement.innerHTML = `
            <h3>${review.author_name}</h3>
            <p>Rating: ${review.rating} estrelas</p>
            <p>${review.text}</p>
            <hr>
          `;
          reviewsContainer.appendChild(reviewElement); // Adiciona a avaliação ao container
        });
      } else {
        reviewsContainer.innerHTML = '<p>Sem avaliações disponíveis no momento.</p>';
      }
    } else {
      document.getElementById("reviews").innerHTML = "<p>Erro ao carregar as avaliações.</p>";
    }
  });
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const observerOptions = {
    root: null,      // Usa o viewport da página como root
    threshold: 0.5   // Quando 50% do vídeo está visível
  };

  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        video.play();  // Toca o vídeo quando entra na seção
      } else {
        video.pause(); // Pausa o vídeo quando sai da seção
      }
    });
  }, observerOptions);

  // Observa a div que contém o vídeo
  observer.observe(document.querySelector('.fundovideo'));
});
